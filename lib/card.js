...
import {
    ...
    getOpenDays, formatScheduleToHuman, parseSchedule, parseRegisterInfo,
    ...
    return `
            <article class="pool-card" id="card-${pool.id}" data-pool-id="${escapeHtml(pool.id)}">
                ...
                <div class="pool-actions">
                    ${pool.regType === 'online' && registerUrl
                        ? `<a href="${escapeHtml(registerUrl)}" target="_blank" class="btn btn-primary" rel="noopener" data-track="reservar" data-pool="${escapeHtml(pool.name)}">Reservar vacante ${icon("external-link")}</a>`
